...
def taper2d(dataset, xsize, ysize, x_spacing, y_spacing):
#前期准备
    x_s = 0
    x_e = 635
    y_s = 0
...
def antitaper2d(dataset, xsize, ysize):
    data = np.zeros(128, 128)
    for i in range(xsize, 128+xsize):
        for j in range(ysize, 128+ysize):
            data[i-xsize][j-ysize] = dataset[i][j]
