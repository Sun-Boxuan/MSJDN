...
class ImageDatasetInpa(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        with open(gt_path, 'r', encoding='utf-8') as f:
            lines = f.readlines()
        pil_gt = [[float(i) for i in line.strip().split(',')] for line in lines]
        ...
        arr_gt = np.array(pil_gt).astype(np.float32)

        mat_max = np.unravel_index(np.argmax(arr_gt, axis=None), arr_gt.shape)
        mat_min = np.unravel_index(np.argmin(arr_gt, axis=None), arr_gt.shape)

        arr_gt = (arr_gt - arr_gt[mat_min]) / (arr_gt[mat_max] - arr_gt[mat_min])
        arr_gt = np.expand_dims(arr_gt, 0)
        # arr_mask = arr_mask.astype(np.float32) / 255.0
        arr_mask = np.expand_dims(arr_mask, 0)
